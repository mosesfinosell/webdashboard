...
export default function StoreMetric({ title, total, ...rest }) {
	...
	useEffect(() => {
		...
		const storeMetricInfo = useSelector(
			(state) =>
				state.businessReducer.storeMetrics?.data.storeMetric
		);
        ...
        <HStack>
          <Box p={9} {...rest}>
            ...
            <Heading fontSize="xl" mt={4}>
              {storeMetricInfo?.noOfProducts || "not availiable"}
            </Heading>
          </Box>
          <Box p={9} {...rest}>
            <Text fontSize="sm">Number of Sales</Text>
            <Heading fontSize="xl" mt={4}>
              {storeMetricInfo?.noOfSales || "not availiable"}
            </Heading>
          </Box>
        </HStack>
        <HStack>
          <Box p={9} {...rest}>
            ...
            <Heading fontSize="xl" mt={4}>
              {storeMetricInfo?.noOfVisitors || "not availiable"}
            </Heading>
          </Box>
          <Box p={9} {...rest}>
            <Text fontSize="sm">Total Sales</Text>
            <Heading fontSize="xl" mt={4}>
              {storeMetricInfo?.totalSales || "not availiable"}
            </Heading>
          </Box>
        </HStack>
      </VStack>
